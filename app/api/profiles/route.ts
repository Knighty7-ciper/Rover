...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch profiles", success: false }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { full_name, title, department, bio, avatar_url, phone, office_location, emergency_contact } = body
